...
public class SwerveSubsystem extends SubsystemBase {
  ...
  public Command aimAtTarget(String limelightName, int tagID) {
    return run(() -> {
      var fiducials = LimelightHelpers.getRawFiducials(limelightName);

      for (var target : fiducials) {
        if (target.id == tagID) {
          double yaw = target.txnc; // Obtém deslocamento horizontal normalizado
          double rotationSpeed = getRotationSpeed(yaw);

          // Comando para girar o robô sem deslocamento lateral
          swerveDrive.drive(new ChassisSpeeds(0, 0, rotationSpeed));
          return;
        }
      }
    });
  ...
  // Controle proporcional para suavizar a rotação
  private double getRotationSpeed(double yaw) {
    double kP = 0.02; // Ajuste conforme necessário
    return -kP * yaw; // Negativo para girar na direção correta
  }
